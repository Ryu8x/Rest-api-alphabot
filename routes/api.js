...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Ryu8xTampan'+'RYU8X--GANS';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'AikaWangy') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'AikaWangy')  return res.json(loghandler.invalidKey)
